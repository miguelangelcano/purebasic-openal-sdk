; *** WAV file format related code
; Requires PureBasic v3.
; Note: For compatibility with PureBasic v4 or later include wave.pb instead.
		...
		If wav_l0 = $46464952 And ReadLong() = $45564157
			...
			While Eof(alWAVE_file) = 0
				...
				Select wav_l0
					...
					; 'fmt '
					Case $20746D66
						ReadData(@wave_fmt.WAVE_FMT, SizeOf(WAVE_FMT))
						...
						Select wave_fmt\wFormatTag
							...
							Case #WAVE_FORMAT_EXTENSIBLE
								dwChannelMask = ReadLong() ; skip cbSize and wReserved
								dwChannelMask = ReadLong()
				...
				If alWAVE_fmt_processed = 1 And alWAVE_data <> 0
					FileSeek(alWAVE_data) : r = 1 : Break
				EndIf

				; Goto next chunk.
				FileSeek(wav_next_chunk)
			Wend
